...
for subj = 1 %1:length(subjects)
    % first load two conditions, and find electrodes where power changes
    ...
    for i=1:length(condition)
        ...
        % the order of loading is important, first movie, so that film_cuts
        % and audio_env are defined.
    ...
    figure(1); h1(3)=nexttile(7,[1 2]); 
    noise_diff = db(model{1}.s2./y2{1}') - db(model{2}.s2./y2{2}');
    plot(noise_diff); axis tight; title('e^2/y^2 power ratio change')
